...
class MultiworkerImageLoader(ImageDataloader[T, L]):
    ...
    def __init__(self, *args, dataset: ImageDataset, batch_size: int, n_workers: int):
        super().__init__(*args, dataset=dataset, batch_size=batch_size)

        self.__work_per_worker = batch_size // n_workers
        self.__pool = Pool(n_workers)
        self.__mapper = MultiworkerImageLoaderMapper(dataset)
