...
class CacheableTensorDataset(Dataset):
    ...
    def __load_cache(self, cache_path: str) -> t.Tuple[Tensor]:
        cache = []
        files = os.listdir(cache_path)

        for file in files:
            tensor = load(f"{cache_path}/{file}")
            cache.append((tensor, os.path.splitext(file)[0]))

        return tuple(cache)

    def __save_cache(self, cache: t.Tuple[Tensor], cache_path: t.Optional[str]) -> None:
        if cache_path is None:
            return
        ...
        for tensor, name in cache:
            save(tensor, f"{cache_path}/{name}.pt")
